...
| 影视详情 | `GET /item/detail?guid=<guid>` | 标题、年份、评分、分类、简介、时长，以及 `episodes/children/versions/sources/list` 等子条目 | `FnosRestClient.mediaDetail()`；失败时详情页仅显示兜底提示，不影响播放 |
...
- 详情页已按 `/item/detail?guid=<guid>` 做非阻塞接入；播放页 `/video/{guid}` 的完整播放源/剧集/版本接口还需要继续抓播放链路。
