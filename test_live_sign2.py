...
def test_sign(url_path):
    s = str(int(time.time() * 1000 % 900000) + 100000)
    c = str(int(time.time() * 1000))
    o = "" # empty for GET without query
    
    l = f"{secret}_{url_path}_{s}_{c}_{o}_{token}"
    sign = md5(l)
    ...
    try:
        with urllib.request.urlopen(req) as resp:
            ...
            print(f"Path '{url_path}':", data.get('code'), data.get('msg', 'Success!'))
            if data.get('code') != 5000:
                return True
    except Exception as e:
        print(f"Path '{url_path}': ERROR", e)
    return False
...
print("Testing live Authx logic...")
