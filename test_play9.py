...
item_guid = first.get('guid')

print("Trying media_guid...")
res = request_api("POST", "/v/api/v1/play/info", body=json.dumps({"media_guid": media_guid}))
if res: print(json.dumps(res, ensure_ascii=False)[:300])

print("Trying video_guid...")
res2 = request_api("POST", "/v/api/v1/play/info", body=json.dumps({"video_guid": video_guid}))
if res2: print(json.dumps(res2, ensure_ascii=False)[:300])

print("Trying guid...")
res3 = request_api("POST", "/v/api/v1/play/info", body=json.dumps({"guid": item_guid}))
if res3: print(json.dumps(res3, ensure_ascii=False)[:300])
