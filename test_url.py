...
def request_api(method, url_path, query="", body=""):
    ...
    try:
        ...
        print(f"Exception for {url_path}: {e}")
        try:
            print(e.read())
        except: pass
        return None
...
item_guid = recent['data'][0].get('guid')

body = json.dumps({"item_guid": item_guid}, separators=(',', ':'))
info = request_api("POST", "/v/api/v1/play/info", body=body)
video_guid = info['data']['video_guid']

print("Testing POST /play/url")
res1 = request_api("POST", "/v/api/v1/play/url", body=json.dumps({"video_guid": video_guid}))
print(res1)
res2 = request_api("POST", "/v/api/v1/play/url", body=json.dumps({"item_guid": item_guid}))
print(res2)
res3 = request_api("POST", "/v/api/v1/item/url", body=json.dumps({"guid": video_guid}))
print(res3)
