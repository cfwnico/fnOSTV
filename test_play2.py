...
first = recent['data']['list'][0]
media_guid = first.get('media_guid')
guid = first.get('guid') # This is the record guid
video_guid = first.get('video_guid')

endpoints_to_test = [
    ("/v/api/v1/play/info", "POST", "", json.dumps({"guid": media_guid})),
    ("/v/api/v1/play/info", "POST", "", json.dumps({"guid": video_guid})),
    ("/v/api/v1/play/info", "POST", "", json.dumps({"media_guid": media_guid})),
    ("/v/api/v1/play/info", "GET", f"guid={media_guid}", ""),
    ("/v/api/v1/play/info", "GET", f"guid={video_guid}", ""),
    ("/v/api/v1/play/info", "GET", f"media_guid={media_guid}", ""),
    
    # Let's also try /item/detail with post and get using video_guid
    ("/v/api/v1/item/detail", "GET", f"guid={video_guid}", ""),
    ("/v/api/v1/item/detail", "POST", "", json.dumps({"guid": video_guid})),
]
